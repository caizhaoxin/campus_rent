...
Page({
  ...
  select_food_type(e) {
    ...
    console.log('物品类型picker发送选择改变，携带值为', this.data.select_food_type);
  },
  ...
  select_time_type(e) {
    ...
    var select_time_type = this.data.time_type[e.detail.value];
    this.setData({
      select_time_type: select_time_type,
    })
    console.log(e.detail.value);
    console.log('租出时间picker发送选择改变，携带值为', this.data.select_time_type);
  },
  /**
   * 生命周期函数--监听页面加载
   */
...
//拼接warnning
    ...
    var havePics = 0; var haveSelect = 0; var haveWrite = 0;
    warnning += '请';
    if (that.data.pics == '') {
      warnning += '上传至少一张图片';
      havePics = 1;
    }
    if (havePics == 1) warnning += ',';
    if (that.data.select_food_type == '选择物品类型 >' || that.data.multiIndex[0] == 99999 ||  that.data.select_time_type == '请选择时间 >'){
      warnning += '填选';
      haveSelect = 1;
    }
    ...
    if (that.data.select_time_type == '请选择时间 >') {
      warnning += '单位、';
    }
    warnning = warnning.replace(reg, ""); 
    if (haveSelect == 1) warnning += ',';
    if (e.detail.value.linkmanName == '' || e.detail.value.goodsName == '' || e.detail.value.rental == '' || e.detail.value.goodsDescription == '' || (e.detail.value.wxID == '' && e.detail.value.phoneNumber == '')
    ) {
      warnning += '填写';
    }
    ...
    if (e.detail.value.goodsDescription == '') {
      warnning += '物品描述、';
    }
    if(warnning!='请'){
      var reg = /、$/gi;//此处是正则
      ...
      wx.request({
        url: "https://huangwenxin.cn/subber/subGoods.php",
        data:
          {
            id: that.data.id,
            openid: app.globalData.openid,
            ...
            unit: that.data.select_time_type,
        ...
        success: function (request_res) {
          app.globalData.haveSubmittedGoods = 1;
